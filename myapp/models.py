...
class SVMModels(models.Model):
    ...
    ecg = models.OneToOneField(
        ECGRecord, on_delete=models.CASCADE, blank=True, null=True
    )
...
def save_contact_message_log(sender, instance, created, **kwargs):
    if created:
        log_date_folder = os.path.join(
            "messages", date.today().isoformat(), "contact_messages"
        )
